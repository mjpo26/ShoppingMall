...
<article id="content">
	<div class="container mainDiv">
		<form action="./productList.sh" name="fr1" method="get"
			...
			<input class="btn btn-primary" type="submit" value="검색하기"> <input
				class="btn btn-secondary" type="reset" value="초기화">
					...
					<tbody>
						...
						<tr>
							<form method="get">
								...
								<!-- 체크박스 -->
								<td><%=articleList.get(i).getItem_title()%></td>
								<!-- 상품명-->
								<td>
									<fmt:formatNumber value="<%=articleList.get(i).getItem_old_price()%>"	pattern="#,###" />
								</td>
								<!-- 판매가-->
								<td>
									<fmt:formatNumber value="<%=articleList.get(i).getItem_old_price() - articleList.get(i).getItem_sel_price()%>"	pattern="#,###" />
								</td>
								<!-- 할인가-->
								...
								<td><select name="display">
										<!-- 진열상태 -->
										<option
											<%if (articleList.get(i).getItem_display().equals("no")) {%>
											selected <%}%>>no</option>
										<option
											<%if (articleList.get(i).getItem_display().equals("yes")) {%>
											selected <%}%>>yes</option></td>

								<td><select name="sales">
										<!-- 판매상태 -->
										<option
											<%if (articleList.get(i).getItem_sales().equals("no")) {%>
											selected <%}%>>no</option>
										<option
											<%if (articleList.get(i).getItem_sales().equals("yes")) {%>
											selected <%}%>>yes</option></td>

								<td><%=articleList.get(i).getItem_stock_count()%></td>
								<!-- 재고 -->
								<td><%=articleList.get(i).getItem_icon1()%></td>
								<!-- 이벤트 -->
								<td><%=articleList.get(i).getItem_category1() + "/" + articleList.get(i).getItem_category2()%></td>
								<!-- 카테고리 -->
								<td><%=articleList.get(i).getItem_Date()%></td>
								<!-- 등록날짜 -->

								<th><input type="hidden" name="Item_code"
									value=<%=articleList.get(i).getItem_code()%>> <input
									class="btn btn-default" type="submit" value="수정"
									formaction="./productUptdate.sh"> <input
									class="btn btn-default" type="button" value="상세수정"
