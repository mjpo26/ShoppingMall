				...
				<table class="table" id="itemtable">
					<tr>
						...
						<td><input type="text" name="title"
							value=<%=article.getItem_title()%>></td>
					</tr>
					<tr>
						<th>판매가(필수)</th>
						<td><input type="text" name="old_price"
							value=<%=article.getItem_old_price()%>> 할인율 : <input
							type="text" name="sale_price" value=<%=sale_price%>>%</td>
					...
					<tr>
						<th>상품상세설명</th>
						<td><textarea id="summernote" name="content2"
								value=<%=article.getItem_content2()%>></textarea></td>
					...
					<tr>
						<th>진열상태</th>
						<td><input type="radio" name="display" value="yes"
							<%if (article.getItem_display().equals("yes")) {%>
							checked="checked" <%}%>>진열함 <input type="radio"
							name="display" value="no"
							<%if (article.getItem_display().equals("no")) {%>
							checked="checked" <%}%>>진열안함</td>
					</tr>

					<tr>
						<th>판매상태</th>
						<td><input type="radio" name="sales" value="yes"
							<%if (article.getItem_sales().equals("yes")) {%>
							checked="checked" <%}%>>판매함 <input type="radio"
							name="sales" value="no"
							<%if (article.getItem_sales().equals("yes")) {%>
							checked="checked" <%}%>>판매안함</td>
					</tr>
					...
					<tr>
					<th> 배송비 입력 </th>
					<td><input type="text" name="delivery_pee" value="50000"></td>
					</tr>
					<tr>
					<th> 상품전체중량 </th>
