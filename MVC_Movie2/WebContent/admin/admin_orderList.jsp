...
<body>
	...
	<form action="./orderList.ol" name="fr1" method="post"
		onsubmit="return pickNullCheck()" id="frm">
		<table border="1">
			<tr>
			...
			<tr>
				<th>상품 검색</th>
				<td colspan='3'><label>상품명</label> <input type="text" size="20"
					name="ItemName"><br> <label>주문자</label> <input
					type="text" size="20" name="Buyer">
			</tr>
			...
			<tr>
				...
				</select> <!-- 	<input type="radio" name="pay" checked="checked" value="">전체 
			...
			<tr>
				...
				</select> <!-- 	<input type="radio" name="delivery"  checked="checked" value="">전체
	...
	<table border="1">
		...
		<tr>
			<th><a href="#"
				onclick="location.href='./orderList.ol?delivery=<%=delivery[0]%>'">
					입금전</a><br><%=delivery1_Count%> EA
			<th><a href="#"
				onclick="location.href='./orderList.ol?delivery=<%=delivery[1]%>'">
					배송전</a><br><%=delivery2_Count%> EA</th>
			<th><a href="#"
				onclick="location.href='./orderList.ol?delivery=<%=delivery[2]%>'">
					배송중</a><br><%=delivery3_Count%> EA
			<th><a href="#"
				onclick="location.href='./orderList.ol?delivery=<%=delivery[3]%>'">
					배송완료</a><br><%=delivery4_Count%> EA</th>
			...
			<table border="1">
				...
				<tr>
					...
					<td><select name="order_pay_status">
							<option
								<%if (articleList.get(i).getOrder_pay_status().equals("입금대기")) {%>
								selected <%}%>>입금대기</option>
							<option
								<%if (articleList.get(i).getOrder_pay_status().equals("입금완료")) {%>
								selected <%}%>>입금완료</option>
					...
					<td><select name="order_delivery_status">
							<option
								...
								selected <%}%>>입금전</option>
							<option
								<%if (articleList.get(i).getOrder_delivery_status().equals("배송전")) {%>
								selected <%}%>>배송전</option>
							<option
								<%if (articleList.get(i).getOrder_delivery_status().equals("배송중")) {%>
								selected <%}%>>배송중</option>
							<option
								<%if (articleList.get(i).getOrder_delivery_status().equals("배송완료")) {%>
								selected <%}%>>배송완료</option>
					</select></td>

					<td><select name="item_status">
							<%-- 						<option <% if(articleList.get(i).getOrder_item_status().equals("취소")){%>selected<%} %>>취소</option> --%>
							<option
								<%if (articleList.get(i).getOrder_item_status().equals("교환")) {%>
								selected <%}%>>교환</option>
							<option
								<%if (articleList.get(i).getOrder_item_status().equals("취소")) {%>
								selected <%}%>>취소</option> 
							<option
								<%if (articleList.get(i).getOrder_item_status().equals("환불")) {%>
								selected <%}%>>환불</option>
							<option
								<%if (articleList.get(i).getOrder_item_status().equals("완료")) {%>
								selected <%}%>>완료</option>
					</select></td>

					<td><select name="bank">
							<option
								<%if (articleList.get(i).getOrder_bank().equals("신한은행")) {%>
								selected <%}%>>신한은행</option>
							<option
								<%if (articleList.get(i).getOrder_bank().equals("국민은행")) {%>
								selected <%}%>>국민은행</option>
							<option
								<%if (articleList.get(i).getOrder_bank().equals("농협은행")) {%>
								selected <%}%>>농협은행</option>
					</select></td>

					<td><select name="payment">
							<option
								<%if (articleList.get(i).getOrder_payment().equals("현금")) {%>
								selected <%}%>>현금</option>
							<option
								<%if (articleList.get(i).getOrder_payment().equals("포인트")) {%>
								selected <%}%>>포인트</option>
							<option
								<%if (articleList.get(i).getOrder_payment().equals("현금+포인트")) {%>
								selected <%}%>>현금+포인트</option>
					</select></td>

					<td><%=articleList.get(i).getOrder_memo()%></td>
					<th><input type="hidden" name="order_idx"
						value="<%=articleList.get(i).getOrder_idx()%>"> <input
						type="submit" value="수정" formaction="./orderUpdate.ol"></th>
				</tr>
				<%
					}
				%>
