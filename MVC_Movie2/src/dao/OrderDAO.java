...
public class OrderDAO {
	...
	public int insertOrder(OrderBean OrderBean) throws Exception {
		...
		try {
			String sql = "INSERT INTO item_order (" + "order_item_code ," + "order_item_title ," + "order_item_count ,"
					+ "order_item_option_color ,"
//                       + "order_item_option_size ,"
					+ "order_item_sel_price ," + "order_item_point ," + "order_plus_point ," + "order_used_point ,"
					+ "order_bank ," + "order_delivery_status ," + "order_pay_status ," // 입금여부
					+ "order_payment ," // 결제방식
					+ "order_memo ," // 메모
...
//            pstmt.setString(4, "M");
			...
			pstmt.setInt(8, OrderBean.getOrder_used_point());
			pstmt.setString(9, OrderBean.getOrder_bank());
//            pstmt.setInt(10, OrderBean.getOrder_used_point()); 
			pstmt.setString(10, "입금전");
			pstmt.setString(11, "입금대기");
			pstmt.setString(12, "현금");
			pstmt.setString(13, OrderBean.getOrder_memo());
			pstmt.setString(14, "완료");
			pstmt.setString(15, OrderBean.getOrder_member_name());
			pstmt.setString(16, OrderBean.getOrder_member_id());
...
//        System.out.println("BoardDAO - isArticleBoardWriter()");
	...
	public int insertReplyArticle(Free_BoardBean article) {
		...
		try {
			...
			sql = "INSERT INTO free_board (" + "free_num," + "free_writer_id," + "free_pass," + "free_subject,"
					+ "free_content," + "free_file1," + "free_ref," + "free_lev," + "free_seq," + "free_readcount,"
					+ "free_replycount," + "free_date," + "free_file2," + "free_file3," + "free_file4,"
	...
	public int updateOrder(OrderBean orderBean) {
		...
		String sql = "UPDATE item_order SET " + "order_pay_status=?," + "order_delivery_status=?,"
				+ "order_item_status=?," + "order_bank=?," + "order_payment=?" + "where order_idx=?";
...
//				+ "if(order_idx=?<1 ,like '%%',?)"
					...
					+ "order_idx like if(?<1,'%%',?)" + "and order_item_title like ifnull(?,'%%') "
					+ "and order_date >= ?" + "and order_date <= ?" + "and order_member_id like ifnull(?,'%%') "
					+ "and order_pay_status like ifnull(?,'%%') " + "and order_delivery_status like ifnull(?,'%%')"
					+ "and order_item_status like ifnull(?,'%%')" + "and order_bank like ifnull(?,'%%')"
					+ "and order_payment like ifnull(?,'%%')" + "ORDER BY order_item_code LIMIT ?,?";
...
//			pstmt.setString(1, ob.getOrder_item_option_color());
	...
	public ArrayList<OrderBean> selectArticleList(String sId) { // 회원 정보 => 장바구니에서 주문
		...
		try {
			...
			while (rs.next()) {
				...
				OrderBean.setOrder_item_title((rs.getString("order_item_title")));
				OrderBean.setOrder_reviewCheck(rs.getString("order_reviewCheck"));
	...
	public OrderBean selectArticleList(int order_item_code) {
		...
		try {
			...
			while (rs.next()) {
				...
				ob.setOrder_item_title((rs.getString("order_item_title")));
				ob.setOrder_reviewCheck(rs.getString("order_reviewCheck"));
	...
	public int updateReview(int order_idx) {
		int updateCount = 0;
		System.out.println("리뷰업데이트값!!!");
		PreparedStatement pstmt = null;

		try {

			String sql = "update item_order set order_reviewCheck=1 where order_idx=?";
			pstmt = con.prepareStatement(sql);
...
//			System.out.println("여기서~~! 아이디랑 ㅏㅏㅏㅏㅏㅏㅏㅏㅏ포인트는!!@"+);
			System.out.println();
			close(pstmt);
