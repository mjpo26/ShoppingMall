...
//         pstmt.setInt(1,pb.getItem_code());
	...
	public int selectListCount(String category, String category2) {
		int listCount = 0;

		PreparedStatement pstmt = null;
		ResultSet rs = null;
		System.out.println("daoㅏ카카테고리1" + category);
		System.out.println("daoㅏ카카테고리2" + category2);
		try {
			if (category.equals("전체") || (category2.equals("의자") && category.equals("전체"))
					|| (category2.equals("침대") && category.equals("전체"))) {
				String sql = "SELECT COUNT(*) FROM Item";
				pstmt = con.prepareStatement(sql);
			} else if (category2.equals("전체") && !category.equals("전체")) {
				String sql = "SELECT COUNT(*) FROM Item where Item_category1=? and Item_display=?  ";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, category);
				pstmt.setString(2, "yes");
			} else {
				String sql = "SELECT COUNT(*) FROM Item where Item_category1=? and Item_display=? and Item_category2=?";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, category);
				pstmt.setString(2, "yes");
	...
	public ArrayList<ItemBean> selectArticleList(int page, int limit, String category, String category2) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			if (category.equals("전체") || (category2.equals("의자") && category.equals("전체"))
				...
				String sql = "SELECT * FROM Item where Item_category1=? and Item_display=? and Item_category2=? LIMIT ?,? ";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, category);
				pstmt.setString(2, "yes");
				pstmt.setString(3, category2);
